...
* AC1: When `HomeScreenViewModel.uiState` is `InitialLoading` (and no previous content/error is shown), a full-screen loading indicator (e.g., centered `CircularProgressIndicator` or a shimmer skeleton grid) is displayed on `HomeScreen`.
* AC2: If the initial API call for curated photos successfully returns an empty list (and `HomeScreenViewModel.uiState` becomes `Empty`), `HomeScreen` displays a user-friendly "empty state" message. The trending photos grid is hidden.
* AC3: If the initial data fetch for `HomeScreen` fails (and `HomeScreenViewModel.uiState` becomes `Error`), a full-screen `ErrorView` with a descriptive message and a "Retry" button is displayed. The trending grid and empty message are hidden.
* AC4: Clicking the "Retry" button on the `ErrorView` triggers `HomeScreenViewModel` to re-attempt fetching the initial set of curated photos, transitioning the UI appropriately (e.g., back to `InitialLoading`).
* AC5: Transitions between the Loading, Content (photo grid), Empty, and Error states on `HomeScreen` are smooth and clear (e.g., using `Crossfade` or `AnimatedVisibility`). Only one of these primary UI states for the trending photos feed is predominantly visible at any given time.
...
* [ ] **Define/Refine `HomeScreenUiState.kt` (if not already done or needs adjustment):**
    ```kotlin
    ...
    // }
    ```
* [ ] **Update `HomeScreenViewModel.kt` to use and manage `HomeScreenUiState`:**
  * [ ] Change state exposure from individual `StateFlow`s (for `isLoadingInitial`, `photos`, `errorState` related to trending photos) to a single `private val _uiState = MutableStateFlow<HomeScreenUiState>(HomeScreenUiState.InitialLoading)` and `val uiState: StateFlow<HomeScreenUiState> = _uiState.asStateFlow()`.
  * [ ] Modify `WorkspaceInitialPhotos()` (and any retry methods like `onManualRefreshTriggered` if they also handle full state reset):
  ...
  * [ ] Ensure `isLoadingNextPage` and `paginationError` remain separate states if they only apply to pagination actions and not the overall screen state for initial load.
* [ ] **Modify `HomeScreen.kt` to Render Based on `HomeScreenUiState`:**
  * [ ] Collect `val currentUiState by viewModel.uiState.collectAsStateWithLifecycle()`.
  * [ ] Use a `Crossfade` (or `AnimatedVisibility` with careful management) to switch the main content area:
    ```kotlin
    ...
    // }
    ```
  * [ ] Implement `ShimmerGridPlaceholderComposable()` for the `InitialLoading` state, using shimmer elements arranged like the staggered grid.
  * [ ] Implement `CenteredMessageComposable(message: String)` for the `Empty` state.
* [ ] Apply consistent animations (from Story 9.3, e.g., `fadeIn` + `slideInVertically`) to the `AnimatedVisibility` or within `Crossfade` for smoother state transitions.
