...
* AC1: Items in the `LazyVerticalStaggeredGrid` on `HomeScreen` have consistent vertical and horizontal spacing applied (e.g., 8.dp between items).
* AC2: `ImageItem`s displayed in the `HomeScreen` grid have visibly rounded corners (e.g., 12.dp radius).
* AC3: A shimmer loading effect is displayed over the placeholder area of each `ImageItem` on `HomeScreen` (which uses `Photo.avgColor` or a default gray as its base) while its respective image is loading via Coil. The shimmer animation should be smooth, visually appealing, and performant.
* AC4: The `LazyVerticalStaggeredGrid` on `HomeScreen` has appropriate `contentPadding` applied, ensuring a margin from the screen edges.
* AC5: The overall loading experience for individual images within the `HomeScreen` grid feels polished and modern, with the shimmer effect enhancing perceived performance by making the loading state less static.
* AC6: The `ImageItem` on `HomeScreen` uses `photo.alt` for its `contentDescription` for accessibility, as per PRD [49], [194], [465] and PO analysis recommendation.
...
* [ ] **Implement Grid Spacing and Content Padding in `HomeScreen.kt`:**
  * [ ] Set `verticalItemSpacing = 8.dp` (or a chosen value like `dimensionResource(id = R.dimen.grid_spacing)`) on `LazyVerticalStaggeredGrid`.
  * [ ] Set `horizontalArrangement = Arrangement.spacedBy(8.dp)` (or chosen value) for `LazyVerticalStaggeredGrid`.
  * [ ] Apply `contentPadding = PaddingValues(horizontal = 12.dp, vertical = 8.dp)` (or chosen values) to `LazyVerticalStaggeredGrid`.
* [ ] **Modify `ImageItem.kt` for Rounded Corners and Accessibility:**
  * [ ] Ensure the root container of `ImageItem` (e.g., a `Card` or `Box` that wraps the `AsyncImage`) has `Modifier.clip(RoundedCornerShape(12.dp))` (or a chosen value like `dimensionResource(id = R.dimen.card_corner_radius)`). This might be conditional if `ImageItem` is used in contexts where rounded corners are not desired, but for `HomeScreen` grid, they are required.
  * [ ] Verify `AsyncImage` uses `contentDescription = photo.alt.ifBlank { "Photo by ${photo.photographer}" }` to fulfill AC6.
* [ ] **Research, Select, and Integrate Shimmer Effect into `ImageItem.kt`:**
  * [ ] **Decision Point:** Choose a shimmer library or decide on custom implementation.
    * If Library: Add dependency to `libs.versions.toml` and `app/build.gradle.kts`. Update `docs/tech-stack.md` with the chosen library and version.
    * If Custom: Plan basic implementation (e.g., animated linear gradient brush).
  * [ ] Modify `ImageItem.kt` to apply the shimmer effect. This typically involves:
    * Having the `AsyncImage`'s `placeholder` slot display a base placeholder (e.g., `Box` with `photo.avgColor` background).
    * Conditionally applying the shimmer modifier to this placeholder `Box` when Coil's `AsyncImagePainter.State` indicates loading.
      ```kotlin
