...
* AC1: When scrolling near the end of `LazyVerticalStaggeredGrid` on `HomeScreen` and conditions are met (not already loading, more can be loaded), `HomeScreenViewModel.loadNextCuratedPage()` is triggered.
* AC2: `HomeScreenViewModel` exposes and correctly manages `isLoadingNextPage: StateFlow<Boolean>` (for pagination of trending photos) and `canLoadMoreCurated: StateFlow<Boolean>`.
* AC3: A loading indicator item is visible at the end of the `LazyVerticalStaggeredGrid` on `HomeScreen` while `HomeScreenViewModel.isLoadingNextPage` is true.
* AC4: New trending photos are appended to the grid on `HomeScreen` seamlessly upon successful loading of the next page.
* AC5: Infinite scrolling for trending photos on `HomeScreen` is smooth and does not cause significant UI jank.
* AC6: If loading the next page of trending photos fails, an inline error message with a "Retry" button is shown at the end of the `HomeScreen`'s grid, and `isLoadingNextPage` is false. Retrying calls `loadNextCuratedPage()` again.
...
* [ ] **Modify `HomeScreenViewModel.kt`:**
    * [ ] Add state variables for trending photo pagination:
    ...
    * [ ] Update `WorkspaceInitialPhotos()` (from Story 10.1):
...
* [ ] **Modify `HomeScreen.kt`:**
    * [ ] Use `val gridState = rememberLazyStaggeredGridState()` and pass it to `LazyVerticalStaggeredGrid`.
    * [ ] Collect `isLoadingInitial`, `isLoadingNextPage`, `canLoadMoreCurated`, `paginationError`, and `photosList` from `viewModel: HomeScreenViewModel`.
    * [ ] Implement scroll detection logic using `LaunchedEffect` and `gridState.layoutInfo`:
      ```kotlin
      ...
      ```
    * [ ] In `LazyVerticalStaggeredGrid`'s content lambda, after the `items` block for `photosList`:
