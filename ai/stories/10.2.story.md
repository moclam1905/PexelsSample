...
**Context:** This story builds on Story 10.1, which set up the initial display of trending photos in a `LazyVerticalStaggeredGrid`. Now, infinite scrolling will be added to fetch and display subsequent pages from the `/v1/curated` Pexels API endpoint. This aligns with PRD v1.2 [37], [182], [453].
...
* Ensure pagination loading state is distinct from initial loading state.
...
* [ ] Add state variables:
* `private var currentCuratedPage = 1`.
...
* [ ] Modify initial fetch in `WorkspaceInitialPhotos()`:
* Reset `currentCuratedPage = 1`, `_canLoadMoreCurated.value = true`, `_paginationError.value = null`.
* On success, set `_canLoadMoreCurated.value = responseData.nextPage != null`.
...
* On success:
* Append new photos to `_photos.value`.
* `_canLoadMoreCurated.value = responseData.nextPage != null`.
* On error: Set `_paginationError.value` with a retryable message. `_canLoadMoreCurated.value` might remain true to allow retry.
* `finally { _isLoadingNextPage.value = false }`.
* [ ] **Modify `HomeScreen.kt`:**
* [ ] Get `gridState = rememberLazyStaggeredGridState()`. Pass to `LazyVerticalStaggeredGrid`.
* [ ] Collect `isLoadingNextPage`, `canLoadMoreCurated`, `paginationError` from ViewModel.
* [ ] Implement scroll detection:
```kotlin
// val buffer = 5 // Items from end to trigger
// val shouldLoadMore = remember {
//     derivedStateOf {
//         val lastVisibleItem = gridState.layoutInfo.visibleItemsInfo.lastOrNull()
//         lastVisibleItem != null && lastVisibleItem.index >= photosList.size - 1 - buffer &&
//         canLoadMoreCuratedValue && !isLoadingInitialValue && !isLoadingNextPageValue && paginationErrorValue == null
//     }
// }
// LaunchedEffect(shouldLoadMore.value) { if (shouldLoadMore.value) viewModel.loadNextCuratedPage() }
```
* [ ] Modify `LazyVerticalStaggeredGrid` items:
```kotlin
// items(...) { /* existing ImageItem */ }
// if (isLoadingNextPageValue) {
//     item(span = StaggeredGridItemSpan.FullLine) { CenteredCircularProgressIndicator(modifier = Modifier.padding(8.dp)) }
// }
// paginationErrorValue?.let { error ->
//    if (!isLoadingNextPageValue) { // Don't show if actively trying to load again
//        item(span = StaggeredGridItemSpan.FullLine) {
//            PaginationErrorItem(errorMsg = error.message, onRetry = { viewModel.loadNextCuratedPage() })
//        }
//    }
// }
```
* [ ] Create `PaginationErrorItem` Composable (displays message and retry button).
* [ ] Create `CenteredCircularProgressIndicator` helper if not already available.
...
* Sets `isLoadingNextPage` true/false.
* Appends new photos on success.
* Sets `canLoadMoreCurated` based on API response (`next_page` presence).
* Sets `paginationError` on failure.
* Handles guard conditions (not loading, can load more).
* **Integration Tests (UI Tests):**
* (To be formalized) Simulate scrolling. Verify `loadNextCuratedPage()` is called. Check for loading indicator and new items, or inline error.
* _(Refer to `testing-strategy.md`.)_
* **Manual/CLI Verification:**
* AC1: Scroll `HomeScreen`. Verify (logs/debugger) `loadNextCuratedPage()` triggers near end if conditions met.
* AC2: Confirm ViewModel states are managed.
* AC3: Observe pagination loading indicator at grid bottom.
* AC4: Verify new items append smoothly.
* AC5: Check for jank during scroll and load.
* AC6: Simulate pagination error (e.g., airplane mode during pagination fetch). Verify inline error & retry appear. Test retry.
