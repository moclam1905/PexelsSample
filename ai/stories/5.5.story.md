...
-   [ ] **Review `SearchViewModel.kt` state management:**
    -   [ ] Ensure `isLoading` is true *only* during the initial fetch for a new search query (or first load on screen).
    -   [ ] Ensure `isLoadingMore` is true *only* during pagination fetches.
    -   [ ] Ensure both are set to `false` in `finally` blocks or after success/error is fully processed.
    -   [ ] Ensure `photos`, `isResultsEmpty`, `errorState` are updated *after* loading flags are appropriately managed or in a way that UI can derive the correct view. For instance, when `isLoading` becomes true for a new search, `photos` list for previous search and `isResultsEmpty` should be cleared/reset.
-   [ ] **Refine `SearchResultsScreen.kt` loading display:**
    -   [ ] When `viewModel.isLoading` is true (initial load for the current query context):
        -   Display a centered `CircularProgressIndicator`.
        -   Ensure previous results (if any from a *different* query), "empty results" message, or error messages are hidden.
    -   [ ] The pagination indicator (`isLoadingMore`) logic from Story 3.4 should be reviewed:
        -   Ensure it appears at the bottom of the grid and doesn't interfere with the main content area or initial loading indicator.
    -   [ ] Consolidate conditional logic for showing content vs. loading vs. error vs. empty:
        ```kotlin
        ...
        // }
        ```
-   [ ] **Review `ImageDetailScreen.kt` and `ImageItem.kt`:**
    -   [ ] Confirm Coil's `AsyncImage` `placeholder` composable/painter provides a clear indication of loading for individual images.
    -   [ ] Ensure the placeholder is not jarring and fits the item's dimensions.
