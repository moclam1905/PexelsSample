...
* [ ] **Modify `HomeScreenViewModel.kt`:**
    ...
    * [ ] Add `private val _snackbarEvent = MutableSharedFlow<String>()` (for one-time messages), `val snackbarEvent: SharedFlow<String> = _snackbarEvent.asSharedFlow()`.
    * [ ] Create/Update `fun onManualRefreshTriggered()` (or a more specific name like `refreshTrendingPhotos()`):
        ...
        * Clear main `_errorState.value = null` (if using a central error state for full screen errors, as this refresh might fix it).
        * `viewModelScope.launch { ... }`:
            * `try`:
                ...
                * On success (`ResultWrapper.Success<PexelsSearchResponseDto>`):
                    * `currentCuratedPage = 1` (reset pagination).
                    * `_photos.value = mappedNewPhotos` (replace list).
                    * `_canLoadMoreCurated.value = responseData.nextPage != null`.
                    * `_paginationError.value = null` (clear any previous pagination errors).
                    * `_snackbarEvent.emit("Trending photos updated.")`.
                * On error (`ResultWrapper.GenericError`, `ResultWrapper.NetworkError`):
                    * `_snackbarEvent.emit("Failed to refresh. ${error.message ?: "Please try again."}")`.
                    * (Optionally, if the `photos` list becomes empty due to this error, the main screen state should reflect an error via `_errorState` for `ErrorView` if desired, as per Story 10.6).
            * `finally { _isRefreshingManual.value = false }`.
* [ ] **Modify `HomeScreen.kt`:**
    * [ ] Ensure `Scaffold` is used. Get or remember `snackbarHostState = remember { SnackbarHostState() }`. Add `SnackbarHost(hostState = snackbarHostState)` to the `Scaffold`.
    * [ ] In the `TopAppBar`'s `actions` slot (which also contains the search icon from updated Story 10.1), add the refresh `IconButton`:
      ```kotlin
      ...
      // }
      ```
    * [ ] Use `LaunchedEffect` to observe `viewModel.snackbarEvent` and show SnackBar:
      ```kotlin
